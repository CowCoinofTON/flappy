...
const cow = 
{
    ...
    draw : function() 
    {
        let cow = this.animation[this.frame];

        // Saving the state of the canvas so only the cow rotates
        ctx.save();
        // Translation from the (0, 0) origin to the cow orgin so the centre of rotation is the centre of the cow
        ...
        if(state.current != state.home)
        {
            ctx.drawImage(
                            sprite_sheet, 
                            cow.spriteX, cow.spriteY, 
                            cow.spriteW, cow.spriteH, 
                            -this.w/2, -this.h/2, // Centering the cow
                            this.w, this.h
                         );
        }
    ...
    update: function() 
    {
        // The cow must flap slowly on get ready state
        ...
        {
            // Reset cow's position after game over
        ...
        else
        {
            ...
            {
                // cow position when it collides with the foreground
            ...
            // If the speed is greater than the jump, means the cow is falling down
            if(this.speed >= this.jump)
            {
                this.rotation = 90 * DEGREE;
                // When cow dies, stop flapping animation
                this.frame = 0;
            }
            else
            {
...
const pipes =
{
    ...
    update : function()
    {
        ...
        for(let i = 0; i < this.position.length; i++)
        {
            ...
            // COLLISION DETECTION
            // Top pipe
            if(cow.x + cow.radius_x > p.x && cow.x - cow.radius_x < p.x + this.w &&
               cow.y + cow.radius_y > p.y && cow.y - cow.radius_y < p.y + this.h)
            {
                state.current = state.gameOver;
                if(!mute)
                {
            ...
            // Bottom pipe
            if(cow.x + cow.radius_x > p.x && cow.x - cow.radius_x < p.x + this.w &&
               cow.y + cow.radius_y > bottomYPos && cow.y - cow.radius_y < bottomYPos + this.h)
            {
                state.current = state.gameOver;
                if(!mute)
                {
            ...
            // Top pipe if cow is out of canvas
            if(cow.x + cow.radius_x > p.x && cow.x - cow.radius_x < p.x + this.w &&
               cow.y <= 0)
            {
                state.current = state.gameOver;
            ...
            // Update score when the cow passes a pipe
            if (p.x + this.w < cow.x - cow.radius_x && !p.scored) 
            {
                score.game_score++;
                if(!mute)
                {
...
const score = 
{
    ...
    scoreReset : function()
    {
			...
			$.ajax({
                url: 'score.php?update',
                method: 'get',
				data: this.game_score,     /* Данные передаваемые в массиве */
				success: function(data){   /* функция которая будет выполнена после успешного запроса.  */
					console.log(data);
					var json = $.parseJSON(data);
					alert('Вы получили +'+json.score+' COW');
				}
            });
			this.game_score = 0;
...
function canvasScale() 
{
    ...
    // cow
    cow.x = cvs.width * 0.290;
    cow.y = cvs.height * 0.395;
    cow.w = cvs.width * 0.117;
    cow.h = cvs.height * 0.059;
    cow.gravity = cvs.height * 0.0006;
    cow.jump = cvs.height * 0.01;
    cow.radius_x = cvs.width * 0.052;
    cow.radius_y = cvs.height * 0.026;
    ...
    // cow
    home.cow.x = cvs.width * 0.803;
    home.cow.y = cvs.height * 0.294;
    home.cow.w = cvs.width * 0.117;
    home.cow.h = cvs.height * 0.059;
    // Studio Name
...
function update() 
{
    // Update position and state of cow, foreground and pipes only if game isn't paused
    if(!gamePaused)
    {
        cow.update();
        foreground.update();
